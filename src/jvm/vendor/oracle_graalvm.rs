...
fn extract_latest_versions(document: &Html) -> Vec<String> {
    ...
    let h_id = Selector::parse("h3[id^='graalvmjava']").unwrap();
...
mod test {
    ...
    fn test_replace_with_latest_version() {
        for (actual, expected) in [
            (
                "https://download.oracle.com/graalvm/24/latest/graalvm-jdk-24_linux-aarch64_bin.tar.gz",
                "https://download.oracle.com/graalvm/24/latest/graalvm-jdk-24.0.1_linux-aarch64_bin.tar.gz",
            ),
            (
                "https://download.oracle.com/graalvm/21/latest/graalvm-jdk-21_linux-aarch64_bin.tar.gz",
                "https://download.oracle.com/graalvm/21/latest/graalvm-jdk-21.0.7_linux-aarch64_bin.tar.gz",
            ),
        ] {
    ...
    fn test_extract_latest_versions() {
        let html = r#"
            <html>
                <body>
                    <h3 id="graalvmjava24">GraalVM for JDK 24.0.1 downloads</h3>
                    <h3 id="graalvmjava24">GraalVM for JDK 21.0.7 downloads</h3>
                </body>
            </html>
        "#;
